...
tcnn::cpp::Module* check_collision_training(std::string directoryPath){
    ...
    for (int i = 0; i < data.size(); i++) {
        ...
        targets[2 * i + 0] = datpoint.empty;
        targets[2 * i + 1] = datpoint.non_empty;
        targets[2 * i + 2] = datpoint.door1;
        targets[2 * i + 3] = datpoint.door2;
        targets[2 * i + 4] = datpoint.door3;
        targets[2 * i + 5] = datpoint.door4;
...
//    size_t newFeaturesSize = (features.size() / 128) * 128;
//    features.resize(newFeaturesSize);
